...
export abstract class MessageServer<T> {
  constructor(private readonly wsServer: WebSocket.Server) {
    ...
    this.wsServer.on('error', this.cleanupClients);
  }
  ...
  protected broadcastExcept(currentClient: WebSocket, message: Readonly<T>): void {
    this.wsServer.clients.forEach(client => {
      if (this.isAlive(client) && client !== currentClient) client.send(JSON.stringify(message));
    });
  }
  ...
  private readonly cleanupClients = (): void => {
    this.wsServer.clients.forEach(client => {
      if (this.isDead(client)) this.wsServer.clients.delete(client);
    });
  };
